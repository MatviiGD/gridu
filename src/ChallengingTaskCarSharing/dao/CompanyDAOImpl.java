...
public class CompanyDAOImpl implements CompanyDAO {
    ...
    public void addCompany(Company company) {
        try (var statement = database.connection.prepareStatement("INSERT INTO company(name) VALUES ?;")) {
    ...
    public List<Company> getAllCompanies() {
        var companies = new LinkedList<Company>();
        try (var statement = database.connection.prepareStatement("SELECT * FROM company;")) {
            var rs = statement.executeQuery();
            while (rs.next()) companies.add(new Company(rs.getInt("id"), rs.getString("name")));
        } catch (SQLException e) {
